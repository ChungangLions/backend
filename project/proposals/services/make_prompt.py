...
def generate_proposal_from_owner_profile(
    *,
    ...
    1) partnership_field(= 제휴 유형)의 값을 산출할때, 다음과 같은 사항을 참고할 것
    ...
    a. partnership_field(= 제휴 유형)이 할인형 이라면, 혜택에 할인율을 포함하되, 마진율x0.7% 와 20% 사이의 값이어야 함.
    b. partnership_field(= 제휴 유형)이 서비스제공형 이라면, 서비스 제공을 포함하되, 제공 품목은 추가 제공 가능 서비스 중에서 하나를 골라서 제공.
    c. partnership_field(= 제휴 유형)이 타임형 이라면, 혜택에 한산 시간대에 서비스 제공을 포함함.
    d. partnership_field(= 제휴 유형)이 리뷰형 이라면, 혜택에 후기 업로드 시 서비스 제공을 포함함.

    expected_effects(= 기대효과)에 잠재 제휴 이용자 수, 기대 매출을 반드시 포함할 것. 만약 partener_ship(= 제휴 유형)이 "리뷰형"이라면, expected_effects(= 기대 효과)에 기대 매출과 노출 건수를 반드시 포함할 것.
    잠재 제휴 이용자 수는 0.52 * 업종별 선호도 * 소속 단위 학생 수 로 산출함. 이때, 업종별 선호도는 업종이 음식점일때 0.23, 
    카페일때 0.21, 술집일때 0.07로 계산함.
    기대 매출은 잠재 이용자 수 * 평균 인당 매출로 산출함.
